...
int main(int argc, char* argv[]) {
  ...
  switch(which) {
    ...
    case 7:
      cilk_spawn increment_i(&stack_data); // write, read race
      mult_double(&stack_data);
      cilk_sync;
      assert(__cilksan_error_count() == 3);
      break;

    case 8:
      cilk_spawn mult_double(&stack_data);
      increment_i(&stack_data); // write, read race
      cilk_sync;
      assert(__cilksan_error_count() == 3);
      break;
    ...
    case 9:
      cilk_spawn mult_double(&stack_data);
      update_str(&stack_data, 3, 8); // write, write race
      cilk_sync;
      assert(__cilksan_error_count() == 2);
      break;
    ...
    case 10:
      cilk_spawn increment_i(&stack_data); // write, read race
      read_double(&stack_data);
      cilk_sync;
      assert(__cilksan_error_count() == 1);
      break;

    case 11:
      cilk_spawn read_double(&stack_data);
      increment_i(&stack_data); // read, write race
      cilk_sync;
      // to be sure that compiler doesn't optimize it away.
      global_int = stack_data.i; 
      assert(__cilksan_error_count() == 1);
      break;

    case 12:
      cilk_spawn read_double(&stack_data);
      update_str(&stack_data, 3, 8); // read, write race
      cilk_sync;
      assert(__cilksan_error_count() == 1);
      break;

    case 13:
      cilk_spawn update_str(&stack_data, 8, 11);
      read_str(&stack_data, 10, 12); // write, read race
      cilk_sync;
      assert(__cilksan_error_count() == 1);
      break;
    ...
    case 16:
      cilk_spawn update_str(&stack_data, 8, 12); // no race
      mult_double(&stack_data);
      cilk_sync;
      assert(__cilksan_error_count() == 0);
      break;
    ...
    case 17:
      cilk_spawn update_str(&stack_data, 4, 12); // no race
      read_i(&stack_data);
      cilk_sync;
      assert(__cilksan_error_count() == 0);
      break;
    ...
    case 18:
      cilk_spawn update_str(&stack_data, 3, 5); // no race 
      update_str(&stack_data, 5, 8);
      cilk_sync;
      assert(__cilksan_error_count() == 0);
      break;
